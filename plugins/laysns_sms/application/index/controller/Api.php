...
//打赏
    ...
    function send_sms()
    {
        ...
        $params = explode(",", $smsConfig['params']);
        $paramArr = [];
        foreach ($params as $v) {
            $_arr = explode("=", $v);
        ...
        //获取结果
        $paramArr['content']    =   "【" . $this->site_config['site_title'] . "】验证码：" . $code . "，您正在绑定" . $this->site_config['site_title'] . "手机号，请5分钟内完成验证。如非本人操作，请忽略本短信。";
        $paramArr['to'] =   $mobile;
        $htd = new Http();
        $data = $htd->get_curl($smsConfig['apiurl'],$paramArr);
