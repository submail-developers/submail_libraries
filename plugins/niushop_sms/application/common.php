 ...
 */
function aliSmsSendOld($appid, $secret, $signName, $smsParam, $send_mobile, $template_code)
{
    $data['appid'] = $appid; //短信平台帐号
    $data['signature'] = $secret; //短信平台密码
    $data['content'] = $signName;//要发送的短信内容
    $data['to'] = $send_mobile;//要发送短信的手机号码
    ...
    $result = file_get_contents("https://api.mysubmail.com/message/xsend/", false, $context);
