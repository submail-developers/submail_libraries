...
class sms
{
	...
	*/
	function send($phones, $msg, $send_date = '', $send_num = 1, $sms_type = '', $version = '1.0')
	{
		...
		$api_url = "https://api.mysubmail.com/message/send";
		foreach ($contents as $key => $val) {
			$send_str['to'] = trim($val['phones']);
			$send_str['content'] = $val['content'];
			$send_str['appid'] = $GLOBALS['_CFG']['赛邮云账号'];
			$send_str['signature'] = $GLOBALS['_CFG']['赛邮云密码'];
			$result = $this->post_data($api_url, $send_str);
			// $result = file_get_contents($api_url.http_build_query($send_str));
			//$result = $rall->parsed();
	...
	{
		$sql = 'SELECT `value` FROM ' . $this->ecs->table('shop_config') . " WHERE `code` = 'certificate'";
