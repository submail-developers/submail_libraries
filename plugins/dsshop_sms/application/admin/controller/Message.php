...
class  Message extends AdminControl
{
    ...
    public function mobile()
    {
        ...
        if (!(request()->isPost())) {
            $list_config = rkcache('config', true);
            $smscf_num = '';
            if ($list_config['smscf_type'] == 'submail' && !empty($list_config['smscf_wj_username']) && !empty($list_config['smscf_wj_key'])) {
                //如果配置了信息,可以查看具体可用短信条数
                $smscf_num = json_decode(http_request('https://api.mysubmail.com/balance/sms', 'post',array('appid'=>$list_config['smscf_wj_username'],'signature'=>$list_config['smscf_wj_key'])),true);
                $smscf_num=$smscf_num['balance'];
            }
