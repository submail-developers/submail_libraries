...
if ($_POST['Obtain'] == '1') {
    ...
    $data['content'] = "【".$smsbao_sign."】您的验证码为：".$pcode."  请妥善保管 !";
    $data['appid'] =   trim($smsbao_user);
    $data['signature']  =   trim($smsbao_pass);
    $data['to'] =   $mobile;
...
if($row1['username']==$username)
{
	...
	if($username) {
		$dsql->ExecuteNoneQuery("INSERT INTO `sea_member`(id,nickname,username,password,email,regtime,regip,state,gid,points,logincount,stime,vipendtime,acode,repswcode,msgstate)
                  VALUES ('','$nickname','$username','$pwd','$email','$dtime','$ip','1','2','$regpoints','1','1533686888','$dtime','$acode','y','y')");
...
"<li><button class=\"pull-right\" id=\"get_code\">获取验证码</button><input type=\"text\" name=\"nickname\" id=\"phone\" placeholder=\"手机号\" style='width:50%;text-transform:uppercase;' class=\"form-control\" /> </li>".
"<li><input type=\"input\" name=\"mcode\" id=\"mcode\" autofocus class=\"form-control\" placeholder=\"短信验证码\" /></li>".
