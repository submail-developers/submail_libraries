...
class EduCloudController extends BaseController
{
    ...
    private function submail_post($api, $data)
    {
        $ch = curl_init();
    ...
    private function sendSms(Request $request, $smsType)
    {
        ...
        $smsapi = "https://api.mysubmail.com/message/send.json";
        $user = $this->setting('auth.user_sms'); //短信平台帐号
        $sing = $this->setting('auth.sing_sms');
        $pass = $this->setting('auth.pwd_sms');
        $content="【".$sing."】".$description.'验证码为'.$smsCode;//要发送的短信内容
    
    $send['appid'] = $user;
        $send['sign_type'] = 'normal';
        $send['content'] = $content;
        $send['to'] = trim($to);
        $send['signature'] = $pass;
    $ret = $this->submail_post($smsapi,$send);
        if ($ret['status']!= 'success' ) {
            return array('error' => sprintf('发送失败, %s', $ret['msg']));
        }
